"""
===================================================================
INVESTMENT PRO BOT - DEPOSIT FLOW (Fixed Version)
===================================================================
Flow:
...
class InvestmentBot:
    ...
    async def send_main_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE, edit: bool = False):
        ...
        # FIXED: Properly handle edit vs new message
        if edit and update.callback_query:
            try:
                ...
                await update.callback_query.message.reply_text(text, reply_markup=Keyboards.main_menu())
        else:
            if update.message:
    ...
    async def back_to_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # FIXED: Always try to edit if callback_query exists, otherwise send new
        try:
            await self.send_main_menu(update, context, edit=True)
        except Exception as e:
            logger.error(f"Error in back_to_menu edit: {e}")
...
def main():
    ...
    # Global back button - NOTE: This should be BEFORE conversation handlers
    # But it won't work inside conversations, which is why we need it in fallbacks too
    ...
    invest_conv = ConversationHandler(
        ...
        states={
            SELECTING_PLAN: [
                ...
                CallbackQueryHandler(bot.back_to_menu, pattern="^back_menu$")  # Back button in plan selection
            ],
            ENTERING_AMOUNT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, bot.amount_entered),
                CallbackQueryHandler(bot.back_to_menu, pattern="^back_menu$")  # Back button while entering amount
            ],
            ENTERING_TRANSACTION_ID: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, bot.transaction_id_entered),
                CallbackQueryHandler(bot.back_to_menu, pattern="^back_menu$")  # Back button while entering TXID
            ],
            CONFIRMING_DEPOSIT: [
                ...
                CallbackQueryHandler(bot.back_to_menu, pattern="^back_menu$")  # Back button at confirmation
            ]
        },
        fallbacks=[
            CommandHandler("start", bot.start),
            CallbackQueryHandler(bot.back_to_menu, pattern="^back_menu$"),  # FIXED: Added back_menu to fallbacks
    ...
    print("=" * 60)
    print("INVESTMENT PRO BOT - DEPOSIT FLOW READY (FIXED VERSION)")
    print("=" * 60)
